...
const Layout = ({ children }) => {
  ...
  const navigation = [
    ...
    ...(isAdmin ? [{ name: 'QR Generator', href: '/qr-generator', icon: QrCode }] : []),
    ...(isAdmin ? [{ name: 'Users', href: '/users', icon: Users }] : []),
    ...(isSuperAdmin ? [{ name: 'Admin Panel', href: '/admin', icon: BarChart3 }] : []),
    ...(isSuperAdmin ? [{ name: 'Settings', href: '/settings', icon: Settings }] : []),
  ];
      ...
      <div className={`fixed inset-0 z-50 lg:hidden ${sidebarOpen ? 'block' : 'hidden'}`}>
        ...
        <div className="fixed inset-y-0 left-0 flex w-64 flex-col bg-white">
          <div className="flex h-16 items-center justify-between px-4">
            <h1 className="text-xl font-semibold text-gray-900">QR Attendance</h1>
            <button
